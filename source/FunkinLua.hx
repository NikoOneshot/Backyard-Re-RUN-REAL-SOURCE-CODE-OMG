...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "windowTweenX", function(tag:String, value:Dynamic, duration:Float, ease:String) {
				lePlayState.modchartTweens.set(tag, FlxTween.tween(Lib.application.window, {x: value}, duration, {ease: getFlxEaseByString(ease),
		...
		Lua_helper.add_callback(lua, "windowTweenY", function(tag:String, value:Dynamic, duration:Float, ease:String) {
				lePlayState.modchartTweens.set(tag, FlxTween.tween(Lib.application.window, {y: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						lePlayState.callOnLuas('onTweenCompleted', [tag]);
						lePlayState.modchartTweens.remove(tag);
					}
...
class DebugLuaText extends FlxText
{
	...
	public function new(text:String, parentGroup:FlxTypedGroup<DebugLuaText>) {
		...
		setFormat(Paths.font("comic.ttf"), 20, FlxColor.BLUE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scrollFactor.set();
		borderSize = 1;
	}
